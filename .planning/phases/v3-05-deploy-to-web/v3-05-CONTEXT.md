...
- v3.0 Quick Wins (Plans 0.5a-4) — shipped: auth middleware, config, Procfile, render.yaml, webhook support all exist
...
- `render.yaml` — Render blueprint with all env var declarations
...
3. Deploy to hosting platform (Render recommended — render.yaml exists, $7/mo Starter for always-on)
4. Set env vars on hosting platform
5. Verify: dashboard loads from phone, Telegram webhook works, scheduler fires agents
6. Optional: custom domain

## Platform Decision
Render Starter ($7/mo) — always-on, auto-deploy from GitHub, render.yaml blueprint ready.
