-- libSQL Schema for Promaia
-- Run this script to initialize the local libSQL / Turso database
...
CREATE TABLE IF NOT EXISTS gmail_content (
    ...
    is_latest_in_thread INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS email_drafts (
    ...
    pertains_to_me INTEGER DEFAULT 1,
...
-- Uses libSQL's native F32_BLOB(768) for gemini-embedding-001 vectors
-- ============================================================
...
-- DiskANN index for fast approximate nearest neighbor search (cosine distance)
CREATE INDEX IF NOT EXISTS idx_content_embeddings_vector
ON content_embeddings(libsql_vector_idx(embedding, 'metric=cosine'));
...
ON property_embeddings(libsql_vector_idx(embedding, 'metric=cosine'));
