...
`},Ou=(e,t,r,i,a=!1)=>{let[n,s,o,u]=i,l=B(i[0].type.tensor);return`
  ...
  }`};return{name:"GroupedConv",shaderCache:{hint:`${t.cacheKey}_${d}`,inputDependencies:f},getRunData:()=>({outputs:[{dims:i?i(r):r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(h/64)},programUniforms:m}),getShaderSource:w}},Vu=(e,t,r,i)=>{let a=e.length>2,n=O(r[3]),s=O(r[2]),o=N.size(r)/n/s,u=[e[0].dims[0],e[0].dims[1],e[0].dims[2],e[0].dims[3]/n],l=[e[1].dims[0],e[1].dims[1],e[1].dims[2],e[1].dims[3]/n],p=[r[0],r[1],r[2],r[3]/n],d=[{type:12,data:o},{type:6,data:[t.strides[0],t.strides[1]]},{type:6,data:[t.pads[0],t.pads[1]]}];Pr(t,d),d.push(...k(u,l,p));let h=(s-1)*t.strides[1]+l[1],m=f=>{let w=K("output",e[0].dataType,p.length,n),$=B(w.type.tensor),_=Dr(t,w.type.value,$),y=A("x",e[0].dataType,u.length,n),S=A("w",e[1].dataType,l.length,n),v=[y,S];a&&v.push(A("b",e[2].dataType,e[2].dims,n));let E=a?"value += b[output_channel];":"",z=[{name:"output_size",type:"u32"},{name:"strides",type:"i32",length:2},{name:"pads",type:"i32",length:2}];return Ur(t,z),`
...
`)}
  ...
  }`};return e.compute({name:"InstanceNormComputeChannelScaleShift",shaderCache:{hint:`${u};${o};${h}`,inputDependencies:w},getRunData:()=>({outputs:[{dims:f,dataType:1}],dispatchGroup:{x:d},programUniforms:$}),getShaderSource:_},{inputs:[t,r,i],outputs:[-1]})[0]},md=(e,t,r)=>{let i=t[0].dims,a=i,n=2,s=i[0],o=i[1],u=N.sizeFromDimension(i,n),l=O(u),p=N.size(a)/l,d=Kn(e,t[0],t[1],t[2],s,u,o,r.epsilon),h=[s,o,u/l],m=[s,o],f=["type","none"],w=$=>{let _=A("x",t[0].dataType,h.length,l),y=A("scale_shift",1,m.length,2),S=K("output",t[0].dataType,h.length,l),v=[_,y,S];return`
  ...
  }`};e.compute({name:"InstanceNormalization",shaderCache:{hint:`${l}`,inputDependencies:f},getRunData:()=>({outputs:[{dims:a,dataType:t[0].dataType}],dispatchGroup:{x:Math.ceil(p/64)},programUniforms:[{type:12,data:p},...k(h,m,h)]}),getShaderSource:w},{inputs:[t[0],d]})},gd=(e,t,r)=>{let i=t[0].dims,a=i,n=i[0],s=i[i.length-1],o=N.sizeFromDimension(i,1)/s,u=O(s),l=N.size(a)/u,p=[{type:12,data:o},{type:12,data:Math.floor(s/u)}],d=["type","type"],h=!1,m=[0,i.length-1];for(let _=0;_<i.length-2;_++)h=h||i[_+1]!==1,m.push(_+1);h=h&&i[i.length-1]!==1;let f=h?e.compute(Ye(e.inputs[0],m),{inputs:[e.inputs[0]],outputs:[-1]})[0]:e.inputs[0].reshape(Array.from({length:i.length},(_,y)=>i[m[y]])),w=Kn(e,f,t[1],t[2],n,o,s,r.epsilon),$=_=>{let y=B(t[0].dataType),S=u===1?"vec2f":`mat${u}x2f`,v=M=>{let L=M===0?"x":"y",H=u===1?"f32":`vec${u}f`;switch(u){case 1:return`${y}(${H}(scale.${L}))`;case 2:return`vec2<${y}>(${H}(scale[0].${L}, scale[1].${L}))`;case 4:return`vec4<${y}>(${H}(scale[0].${L}, scale[1].${L}, scale[2].${L}, scale[3].${L}))`;default:throw new Error(`Not supported compoents ${u}`)}},E=A("input",t[0].dataType,t[0].dims,u),z=K("output",t[0].dataType,a,u);return`
