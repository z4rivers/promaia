                            ...
                            if response.tool_call is not None:
                                ...
                                for ft in response.tool_call.function_calls:
                                    if ft.name == "save_conversation_memory":
                                        ...
                                        args = ft.args
                                        logger.info(f"Calendar tool called with args: {args}")
                                        try:
                                            from promaia.gcal.google_calendar import GoogleCalendarManager
                                            mgr = GoogleCalendarManager()
                                            auth_result = mgr.authenticate()
                                            logger.info(f"Calendar auth result: {auth_result}")
                                            if auth_result:
                                                event_body = {
                                                ...
                                                logger.info(f"Creating calendar event: {event_body}")
                                                ...
                                                logger.error("Calendar authentication returned False during tool call")
                                                tool_responses.append(types.FunctionResponse(
                                                    name=ft.name,
                                                    id=ft.id,
                                        ...
                                        args = ft.args
                                        logger.info(f"Delete calendar tool called with args: {args}")
                                        try:
                                            from promaia.gcal.google_calendar import GoogleCalendarManager
                                            mgr = GoogleCalendarManager()
                                            if mgr.authenticate():
                                                ...
                                                if events:
                                                    ...
                                                    for ev in events:
                                                        if search_summary in ev.get('summary', '').lower():
                                                            ...
                                                            logger.info(f"Deleted calendar event: {ev.get('summary')}")
                                                ...
                                                logger.error("Calendar auth failed during delete tool call")
                                                tool_responses.append(types.FunctionResponse(
                                                    name=ft.name,
                                                    id=ft.id,
