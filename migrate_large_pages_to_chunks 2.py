#!/usr/bin/env python3
"""
Migration script to re-process existing large pages with chunking.

This script:
1. Queries all pages from unified_content
2. For each page, reads the markdown file
3. Estimates token count
4. If tokens > 6000, chunks the page and re-embeds it
5. Shows progress and statistics
"""
import os
import sys
import json
import logging
from datetime import datetime
from typing import Dict, Any, List

# Add promaia to path
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from promaia.storage.hybrid_storage import get_hybrid_registry
from promaia.storage.vector_db import VectorDBManager
from promaia.storage.page_chunker import chunk_page_content, estimate_tokens

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)


def load_config() -> Dict[str, Any]:
    """Load configuration from promaia.config.json."""
    config_path = "promaia.config.json"
    try:
        with open(config_path, 'r') as f:
            return json.load(f)
    except Exception as e:
        logger.error(f"Failed to load config: {e}")
        sys.exit(1)


def migrate_large_pages(
    max_tokens: int = 6000,
    dry_run: bool = False,
    workspace: str = None,
    database_name: str = None
):
    """
    Migrate existing large pages to use chunking.
    
    Args:
        max_tokens: Token threshold for chunking (default: 6000)
        dry_run: If True, only analyze without making changes
        workspace: Optional workspace filter
        database_name: Optional database filter
    """
    # Load configuration
    config = load_config()
    vector_config = config.get('global', {}).get('vector_search', {})
    
    if not vector_config.get('enabled', False):
        logger.error("Vector search is disabled in config. Please enable it first.")
        sys.exit(1)
    
    # Initialize registry and vector DB
    registry = get_hybrid_registry()
    vector_db = VectorDBManager(chroma_path=vector_config.get('chroma_path', 'chroma_db'))
    
    # Get all pages from unified_content
    logger.info("Querying all pages from unified_content...")
    
    try:
        import sqlite3
        with sqlite3.connect("data/hybrid_metadata.db") as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            
            # Build query with optional filters
            query = "SELECT page_id, file_path, workspace, database_name, created_time FROM unified_content"
            params = []
            where_clauses = []
            
            if workspace:
                where_clauses.append("workspace = ?")
                params.append(workspace)
            
            if database_name:
                where_clauses.append("database_name = ?")
                params.append(database_name)
            
            if where_clauses:
                query += " WHERE " + " AND ".join(where_clauses)
            
            cursor.execute(query, params)
            pages = [dict(row) for row in cursor.fetchall()]
    except Exception as e:
        logger.error(f"Failed to query pages: {e}")
        sys.exit(1)
    
    logger.info(f"Found {len(pages)} pages to analyze")
    
    # Statistics
    stats = {
        'total_pages': len(pages),
        'large_pages': 0,
        'chunked_pages': 0,
        'failed_pages': 0,
        'total_chunks_created': 0,
        'skipped_missing_files': 0,
        'skipped_small_pages': 0
    }
    
    # Process each page
    for i, page in enumerate(pages, 1):
        page_id = page['page_id']
        file_path = page['file_path']
        
        if i % 10 == 0:
            logger.info(f"Progress: {i}/{len(pages)} pages analyzed...")
        
        # Check if file exists
        if not os.path.exists(file_path):
            stats['skipped_missing_files'] += 1
            logger.debug(f"Skipping {page_id}: File not found at {file_path}")
            continue
        
        # Read content
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
        except Exception as e:
            logger.warning(f"Failed to read {file_path}: {e}")
            stats['failed_pages'] += 1
            continue
        
        # Skip empty or very short content
        if not content or len(content.strip()) < 10:
            continue
        
        # Estimate tokens
        estimated_tokens = estimate_tokens(content, provider=vector_db.embedding_provider)
        
        # Check if chunking is needed
        if estimated_tokens <= max_tokens:
            stats['skipped_small_pages'] += 1
            continue
        
        # Found a large page
        stats['large_pages'] += 1
        logger.info(f"\n📄 Large page found: {page_id}")
        logger.info(f"   Tokens: {estimated_tokens} (exceeds {max_tokens})")
        logger.info(f"   Database: {page['database_name']} (workspace: {page['workspace']})")
        logger.info(f"   File: {file_path}")
        
        if dry_run:
            logger.info("   [DRY RUN] Would chunk this page")
            continue
        
        # Chunk the page
        try:
            chunks = chunk_page_content(
                markdown_content=content,
                page_id=page_id,
                block_metadata=None,
                max_tokens=max_tokens,
                provider=vector_db.embedding_provider
            )
            
            if not chunks:
                logger.error(f"   ❌ Failed to generate chunks for {page_id}")
                stats['failed_pages'] += 1
                continue
            
            logger.info(f"   ✅ Generated {len(chunks)} chunks")
            stats['total_chunks_created'] += len(chunks)
            
            # Remove old chunks if they exist
            registry.remove_chunks_for_page(page_id)
            
            # Store chunks in database
            synced_time = datetime.utcnow().isoformat()
            for chunk in chunks:
                chunk_data = {
                    'chunk_id': chunk['chunk_id'],
                    'page_id': page_id,
                    'chunk_index': chunk['chunk_index'],
                    'total_chunks': chunk['total_chunks'],
                    'workspace': page['workspace'],
                    'database_name': page['database_name'],
                    'char_start': chunk['char_start'],
                    'char_end': chunk['char_end'],
                    'estimated_tokens': chunk['estimated_tokens'],
                    'date_boundary': chunk.get('date_boundary'),
                    'parent_file_path': file_path,
                    'created_time': page.get('created_time'),
                    'synced_time': synced_time
                }
                registry.add_page_chunk(chunk_data)
            
            # Prepare metadata
            metadata = {
                'database_name': page['database_name'],
                'workspace': page['workspace'],
                'created_time': page.get('created_time', ''),
                'content_type': page['database_name'],  # Use database_name as content_type
            }
            
            # Re-embed with chunking
            success = vector_db.add_content_with_chunking(
                page_id=page_id,
                content_text=content,
                metadata=metadata,
                chunks=chunks
            )
            
            if success:
                logger.info(f"   ✅ Successfully re-embedded with {len(chunks)} chunks")
                stats['chunked_pages'] += 1
            else:
                logger.error(f"   ❌ Failed to re-embed chunks")
                stats['failed_pages'] += 1
        
        except Exception as e:
            logger.error(f"   ❌ Error processing {page_id}: {e}")
            stats['failed_pages'] += 1
            import traceback
            logger.debug(traceback.format_exc())
    
    # Print summary
    print("\n" + "="*60)
    print("MIGRATION SUMMARY")
    print("="*60)
    print(f"Total pages analyzed:       {stats['total_pages']}")
    print(f"Large pages found:          {stats['large_pages']}")
    print(f"Pages successfully chunked: {stats['chunked_pages']}")
    print(f"Total chunks created:       {stats['total_chunks_created']}")
    print(f"Failed pages:               {stats['failed_pages']}")
    print(f"Skipped (missing files):    {stats['skipped_missing_files']}")
    print(f"Skipped (small pages):      {stats['skipped_small_pages']}")
    print("="*60)
    
    if dry_run:
        print("\n💡 This was a DRY RUN. No changes were made.")
        print("   Run without --dry-run to perform the migration.")


def main():
    """Main entry point."""
    import argparse
    
    parser = argparse.ArgumentParser(
        description="Migrate existing large pages to use chunking"
    )
    parser.add_argument(
        '--max-tokens',
        type=int,
        default=6000,
        help='Token threshold for chunking (default: 6000)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Analyze without making changes'
    )
    parser.add_argument(
        '--workspace',
        type=str,
        help='Filter by workspace'
    )
    parser.add_argument(
        '--database',
        type=str,
        help='Filter by database name'
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
        help='Show detailed debug output'
    )
    
    args = parser.parse_args()
    
    # Set log level
    if args.verbose:
        logging.getLogger().setLevel(logging.DEBUG)
    
    # Run migration
    migrate_large_pages(
        max_tokens=args.max_tokens,
        dry_run=args.dry_run,
        workspace=args.workspace,
        database_name=args.database
    )


if __name__ == "__main__":
    main()
